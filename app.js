...
const st={conectado:false,conectando:false,simbolo:"",mercado:"",estrategia:"rise_fall",modo:"rapido",precios:[],digitos:[],ticks:0,previo:null,pip:null,indicadores:null,resultado:null,resultadoPreparado:null,motorActivo:false,historial:[],vozActiva:true,vozDisponible:("speechSynthesis"in window&&"SpeechSynthesisUtterance"in window)};
let timer=null;
...
function actualizarBotonMotor(){if(!ui.botonEncender)return;ui.botonEncender.disabled=!st.conectado;ui.botonEncender.classList.toggle("activo",st.motorActivo);ui.botonEncender.textContent=st.motorActivo?"⚡ Herramienta encendida":"⚡ Encender herramienta"}
function progreso(){const m=minTicks(),c=Math.min(st.precios.length,m);ui.numeroProgreso.textContent=`${c}/${m}`;ui.barraDatos.style.width=`${Math.min(100,c/m*100)}%`;actualizarBotonMotor();if(!st.conectado){ui.textoProgreso.textContent="Esperando conexión";ui.botonAnalizar.disabled=true;ui.botonAnalizar.textContent="Esperando conexión...";return}if(!st.motorActivo){ui.textoProgreso.textContent="Herramienta apagada";ui.botonAnalizar.disabled=true;ui.botonAnalizar.textContent="Enciende la herramienta";return}ui.botonAnalizar.disabled=false;ui.botonAnalizar.textContent="⚡ Predicción";if(st.precios.length<m){ui.textoProgreso.textContent=`Motor activo · memoria ${st.precios.length}/${m}`;return}ui.textoProgreso.textContent="Motor activo · predicción preparada"}
...
function indicadores(){const p=st.precios;if(p.length<CFG.minimoMotor)return null;const n=Math.min(st.modo==="completo"?30:12,p.length),rec=p.slice(-n),mitad=Math.floor(rec.length/2),a=promedio(rec.slice(0,mitad)),b=promedio(rec.slice(mitad)),cambio=a?((b-a)/a)*100:0;let tend="Lateral";if(cambio>0)tend="Alcista";if(cambio<0)tend="Bajista";const per=st.modo==="completo"?14:8;let rsi=null;if(p.length>=per+1){let g=0,l=0;const d=p.slice(-(per+1));for(let i=1;i<d.length;i++){const x=d[i]-d[i-1];if(x>0)g+=x;else l+=Math.abs(x)}rsi=l===0?(g===0?50:100):100-100/(1+(g/per)/(l/per))}const pm=Math.min(st.modo==="completo"?10:5,p.length-1);const val=pm>0?p.at(-1)-p.at(-1-pm):0;const mom=val>0?"Positivo":val<0?"Negativo":"Neutral";const desv=Math.sqrt(promedio(rec.map(x=>(x-promedio(rec))**2)));const vol=promedio(rec)?desv/promedio(rec)*100:0;const nivel=vol>=.08?"Alta":vol>=.025?"Media":"Baja";return{tendencia:{direccion:tend,cambio},rsi,momentum:{direccion:mom,valor:val,porcentaje:pm>0&&p.at(-1-pm)?val/p.at(-1-pm)*100:0},volatilidad:{nivel,porcentaje:vol}}}
...
function resultado(){
  ...
  if(recientes.length<3)return crearResultado("ESPERAR",45,[],["El motor está reuniendo los primeros dígitos. La operación todavía no es recomendable."]);
...
function actualizarMotorSegundoPlano(){
  if(!st.conectado||!st.motorActivo||st.precios.length<CFG.minimoMotor){st.resultadoPreparado=null;return}
...
function analizar(){if(!st.conectado){progreso();return}if(!st.motorActivo){activarMotor();return}voz("Generando predicción.");pintarIndicadores();actualizarMotorSegundoPlano();const r=st.resultadoPreparado||resultado()||crearResultado("ESPERAR",45,[],["El motor acaba de encender y está reuniendo los primeros datos. Intenta nuevamente en unos segundos."]);mostrar(r);historial(r);log(`Predicción generada: ${r.direccion} con ${r.confianza}% de confianza técnica.`,r.direccion==="ESPERAR"?"advertencia":"exito");voz(r.direccion==="ESPERAR"?"Operación no recomendada. El motor todavía no encuentra una ventaja clara.":`Predicción ${r.direccion}. Confianza técnica ${r.confianza} por ciento. Tienes ${r.vigencia} segundos para realizar la operación.`);cuenta(r)}
function activarMotor(silencioso=false){if(!st.conectado){log("Primero conecta con Deriv.","advertencia");return}st.motorActivo=true;pintarIndicadores();actualizarMotorSegundoPlano();progreso();log("Motor de análisis encendido y trabajando en segundo plano.","exito");if(!silencioso)voz("Herramienta encendida.")}
function alternarMotor(){if(!st.conectado)return;if(st.motorActivo){st.motorActivo=false;st.resultadoPreparado=null;detenerCuenta();progreso();log("Motor de análisis apagado.","advertencia");voz("Herramienta apagada.")}else activarMotor()}
...
function iniciar(){sync();estadoConexion("desconectado","Desconectado");resetDatos();botonVoz();ui.botonConectar.addEventListener("click",conectar);ui.botonDesconectar.addEventListener("click",desconectar);ui.botonEncender.addEventListener("click",alternarMotor);ui.selectorIndice.addEventListener("change",cambioMercado);ui.selectorOperacion.addEventListener("change",cambioOperacion);ui.selectorModo.addEventListener("change",cambioModo);ui.botonAnalizar.addEventListener("click",analizar);ui.botonVoz.addEventListener("click",()=>{st.vozActiva=!st.vozActiva;detenerVoz();botonVoz();if(st.vozActiva)voz("Asistente de voz activado.")});ui.botonLimpiarHistorial.addEventListener("click",()=>{st.historial=[];ui.historialAnalisis.innerHTML='<p class="mensaje-vacio">Todavía no se han generado análisis.</p>';log("Historial de análisis limpiado.")});ui.botonLimpiarRegistro.addEventListener("click",()=>{ui.registroActividad.innerHTML="";log("Registro de actividad limpiado.")});log(`Trading Analyzer V${CFG.version} cargado correctamente.`,"exito")}
