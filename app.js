...
const ui = {
  ...
  capsulaPrediccion: $("capsulaPrediccion"),
  capsulaEstado: $("capsulaEstado"),
  capsulaValor: $("capsulaValor"),
  capsulaPuntaje: $("capsulaPuntaje")
};
...
function predictRiseFall() {
  ...
  if (!indicators || state.prices.length < 6) {
    return noRecommendation(
      "El motor todavía está reuniendo movimientos suficientes para Rise y Fall."
    );
  }
  ...
  const directionScore = (values) => {
    if (values.length < 3) return 0;

    let score = 0;

    for (let index = 1; index < values.length; index++) {
      if (values[index] > values[index - 1]) score++;
      if (values[index] < values[index - 1]) score--;
    }

    return score / (values.length - 1);
  };

  const shortFlow = directionScore(recent6);
  const mediumFlow = directionScore(recent12);
  const longFlow = directionScore(recent30);
  ...
  if (Number.isFinite(indicators.rsi)) {
    if (indicators.rsi >= 56 && indicators.rsi <= 68) {
      bullish += 28;
      reasons.push(
        `El RSI de ${indicators.rsi.toFixed(1)} confirma presión alcista.`
      );
    } else if (indicators.rsi <= 44 && indicators.rsi >= 32) {
      bearish += 28;
      reasons.push(
  ...
  if (shortFlow > 0.25) bullish += 12;
  if (shortFlow < -0.25) bearish += 12;

  if (mediumFlow > 0.20) bullish += 10;
  if (mediumFlow < -0.20) bearish += 10;

  if (longFlow > 0.15) bullish += 8;
  if (longFlow < -0.15) bearish += 8;

  if (
    Math.sign(shortFlow) === Math.sign(mediumFlow) &&
    Math.sign(shortFlow) !== 0
  ) {
    reasons.push("Los movimientos corto y medio están alineados.");
  } else {
    warnings.push("Los movimientos corto y medio no están alineados.");
  }
  ...
  const aligned =
    (bullish > bearish && shortFlow > 0 && mediumFlow >= 0) ||
    (bearish > bullish && shortFlow < 0 && mediumFlow <= 0);

  if (!aligned || difference < 24 || winningScore < 62) {
    return noRecommendation(
      "RSI, momentum, tendencia y flujo de ticks no presentan suficiente coincidencia.",
      reasons
    );
  }
  ...
  return prediction(
    ...
    Math.min(90, 52 + difference * 0.62),
    reasons,
    warnings
  );
}
...
function predictEvenOdd() {
  ...
  return prediction(
    ...
    Math.min(76, 48 + advantage * 1.6),
    reasons,
    [
      "Estrategia experimental: la frecuencia histórica no garantiza el próximo dígito."
    ]
  );
}
...
function predictMatch() {
  ...
  return prediction(
    ...
    reasons,
    [
      "Match es una estrategia experimental y se valida con el siguiente tick."
    ]
  );
}
...
function capsuleTone(direction) {
  if (direction === "NO RECOMENDADO") return "caution";
  if (["FALL", "ODD", "UNDER"].includes(direction)) return "negative";
  return "positive";
}

function hidePredictionCapsule() {
  if (state.capsuleTimer) {
    clearTimeout(state.capsuleTimer);
    state.capsuleTimer = null;
  }

  ui.capsulaPrediccion.classList.add("hiding");
  ui.capsulaPrediccion.classList.remove("visible");
  document.body.classList.remove("capsule-open");

  setTimeout(() => {
    ui.capsulaPrediccion.className = "prediction-capsule";
    ui.capsulaPrediccion.setAttribute("aria-hidden", "true");
  }, 360);
}

function showPredictionCapsule(result) {
  if (!ui.capsulaPrediccion) return;

  if (state.capsuleTimer) {
    clearTimeout(state.capsuleTimer);
  }

  const tone = capsuleTone(result.direction);
  ui.capsulaPrediccion.className =
    `prediction-capsule ${tone}`;

  ui.capsulaEstado.textContent =
    result.direction === "NO RECOMENDADO"
      ? "ESPERAR"
      : "PREDICTION";

  ui.capsulaValor.textContent =
    result.direction === "NO RECOMENDADO"
      ? "NO OPERAR"
      : result.direction;

  ui.capsulaPuntaje.textContent =
    result.direction === "NO RECOMENDADO"
      ? "Sin confirmación suficiente"
      : `Puntaje ${result.score}/100`;

  ui.capsulaPrediccion.setAttribute("aria-hidden", "false");
  document.body.classList.add("capsule-open");

  requestAnimationFrame(() => {
    ui.capsulaPrediccion.classList.add("visible");
  });

  const visibleTime =
    result.direction === "NO RECOMENDADO" ? 2600 : 3400;

  state.capsuleTimer = setTimeout(
    hidePredictionCapsule,
    visibleTime
  );
}

function showObservedResult(hit, pending) {
  ui.panelSenal.className =
    `signal-panel ${hit ? "good" : "bad"}`;
  ...
  ui.prediccionTitulo.textContent =
    hit
      ? "La señal coincidió con el siguiente tick"
      : "La señal no coincidió con el siguiente tick";

  ui.prediccionDireccion.textContent =
    pending.direction;

  ui.vigenciaSenal.textContent =
    "La señal finalizó. Puede generar una nueva predicción.";
  ...
  ui.capsulaPuntaje.textContent =
    hit ? "Resultado positivo" : "Resultado negativo";

  ui.capsulaPrediccion.className =
    `prediction-capsule ${hit ? "positive" : "negative"}`;
  ...
  if (state.capsuleTimer) clearTimeout(state.capsuleTimer);
  state.capsuleTimer = setTimeout(hidePredictionCapsule, 2400);
}
...
function voiceExplanation(result) {
  ...
  parts.push(`Puntaje técnico de ${result.score} sobre cien. Este puntaje no es una garantía.`);
...
function prepareValidation(result) {
  ...
  if (result.strategyKey === "rise_fall") {
    scheduleRiseFallValidation(state.pendingValidation);
  }
}


function clearRiseFallValidationTimer() {
  if (state.riseFallValidationTimer) {
    clearTimeout(state.riseFallValidationTimer);
    state.riseFallValidationTimer = null;
  }
}

function scheduleRiseFallValidation(pending) {
  clearRiseFallValidationTimer();

  state.riseFallValidationTimer = setTimeout(() => {
    state.riseFallValidationTimer = null;
    ...
    const hit =
      pending.direction === "RISE"
        ? state.lastPrice > pending.referencePrice
        : state.lastPrice < pending.referencePrice;
    ...
    log(
      `Resultado Rise/Fall a 10 segundos: ${hit ? "ACIERTO" : "FALLO"}.`,
      hit ? "success" : "warning"
    );

    stopSignal();
    showObservedResult(hit, pending);
  }, CFG.riseFallCountdown * 1000);
}
